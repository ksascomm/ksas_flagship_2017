	...
	if ( ! empty( $frontcover ) ) :?>
		<?php foreach ( $frontcover as $cover ) : ?>
			<div class="issue-cover">
				<div class="card">
					<img class="cover" src="<?php echo $cover->_embedded->{'wp:featuredmedia'}[0]->media_details->sizes->{'medium_large'}->source_url;?>" alt="<?php echo $cover->_embedded->{'wp:featuredmedia'}[0]->alt_text;?>">
				</div>
			</div>
		<?php endforeach;?>
	<?php endif;?>